...
export function RadioPlayer() {
  ...
  const togglePlay = async () => {
    ...
    if (isPlaying) {
      audioRef.current.pause();
      // Clearing the source stops the browser from downloading the stream in the background
      audioRef.current.src = "";
      setIsPlaying(false);
      setError(null);
    } else {
      ...
      setError(null);
      try {
        // Re-assign the stream URL to ensure we start at the 'live edge'
      ...
      <div className="relative bg-card border border-white/10 rounded-2xl p-6 md:p-8 flex flex-col items-center shadow-2xl">
        <div className="w-full flex items-center justify-between mb-8">
          <div className="flex items-center gap-2">
            ...
            <span className={`flex h-2 w-2 rounded-full ${error ? 'bg-destructive' : 'bg-primary'} ${isPlaying && !isLoading ? 'animate-pulse' : 'opacity-40'}`} />
          </div>
          <div className="hidden sm:flex items-center gap-2 text-xs font-mono text-muted-foreground uppercase tracking-widest">
            <span className={`w-2 h-2 rounded-full ${error ? 'bg-destructive' : isPlaying ? 'bg-green-500' : 'bg-orange-500'}`} />
            {error ? 'Offline' : isPlaying ? 'Connected' : 'Ready'}
          </div>
        </div>

        <div className="flex flex-col items-center space-y-8 w-full py-4">
          <div className="relative group">
            <div className={`absolute -inset-6 bg-primary/20 rounded-full blur-2xl transition-all duration-1000 ${isPlaying && !isLoading ? 'opacity-100 scale-125' : 'opacity-0 scale-90'}`} />
            <Button 
              size="icon" 
              onClick={togglePlay}
              disabled={isLoading}
              className={`h-28 w-28 rounded-full shadow-2xl cyan-glow relative z-10 transition-transform hover:scale-105 active:scale-95 ${error ? 'bg-destructive' : 'bg-primary hover:bg-primary/90'}`}
            >
              {isLoading ? (
          ...
          <div className="text-center space-y-3">
            ...
            <p className="text-muted-foreground font-medium italic text-lg tracking-wide">
              {error || '"Empowering the frequency of change, globally."'}
            </p>
          </div>
